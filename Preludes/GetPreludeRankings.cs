...
namespace BgaTmScraperRegistry.Functions
{
    public static class GetPreludeRankings
    {
        ...
        public static async System.Threading.Tasks.Task<IActionResult> Run(
            ...
            ILogger log)
        {
            log.LogInformation("GetPreludeRankings function processed a request");
            ...
            try
            {
                ...
                var rankings = await service.GetPreludeRankingsAsync(filter);

                log.LogInformation("Returning {count} prelude rankings", rankings.Count);
                return new OkObjectResult(rankings);
            }
            catch (Exception ex)
            {
                log.LogError(ex, "Error occurred while getting prelude rankings");
                return new StatusCodeResult(500);
            }
        }

        private static PreludeStatsService.PreludeFilter ParseFilter(HttpRequest req)
        {
            // Helpers
            ...
            var filter = new PreludeStatsService.PreludeFilter
            {
                ...
                Corporation = First(req, "corporation", "corp")
            };

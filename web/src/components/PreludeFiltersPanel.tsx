...
interface PreludeFiltersPanelProps {
  ...
  onFiltersChange: (filters: PreludeDetailFilters) => void;
...
export function PreludeFiltersPanel({
  filters,
  onFiltersChange,
  ...
  const resetFilters = () => {
    const defaultFilters: PreludeDetailFilters = {
      maps: availableMaps,
  ...
  const toggleCorporation = (corporation: string) => {
    const newCorporations = localFilters.corporations.includes(corporation)
      ? localFilters.corporations.filter(c => c !== corporation)
      : [...localFilters.corporations, corporation].sort();

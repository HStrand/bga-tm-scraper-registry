...
namespace BgaTmScraperRegistry.Functions
{
    public static class GetStartingHandStats
    {
            ...
            ILogger log)
        {
            ...
            var target = $"{baseUrl.TrimEnd('/')}/api/startinghands/stats{req.QueryString.Value}";
            ...
            {
                log.LogError("Starting hand stats proxy request timed out ({Target})", target);
                return new ContentResult
                {
            ...
            {
                log.LogError(ex, "Error proxying starting hand stats request to {Target}", target);

...
namespace BgaTmScraperRegistry
{
    public static class GetPlayerParameterStats
    {
            ...
            ILogger log)
        {
            ...
            var target = $"{baseUrl.TrimEnd('/')}/api/GetPlayerParameterStats{req.QueryString.Value}";
            return await BgaTmScraperRegistry.Functions.LeaderboardProxyHelpers.ProxyGet(_httpClient, target, "player parameter stats", log);
        }
    }
}

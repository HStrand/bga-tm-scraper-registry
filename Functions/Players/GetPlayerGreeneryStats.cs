...
namespace BgaTmScraperRegistry
{
    public static class GetPlayerGreeneryStats
    {
            ...
            ILogger log)
        {
            ...
            var target = $"{baseUrl.TrimEnd('/')}/api/GetPlayerGreeneryStats{req.QueryString.Value}";
            return await BgaTmScraperRegistry.Functions.LeaderboardProxyHelpers.ProxyGet(_httpClient, target, "player greenery stats", log);
        }
    }
}
